// Configuration Template - À personnaliser pour chaque cabinet
const CABINET_CONFIG = {
    // OBLIGATOIRE - À modifier pour chaque cabinet
    WEBHOOK_URL: 'agenceefficads/onboarding-test',
    CABINET_NAME: 'AGENCE EFFICADS',
    GESTIONNAIRE_NOM: 'BENHLAL YAZID',
    GESTIONNAIRE_EMAIL: 'benhlalyazid@gmail.com',
    
    // OPTIONNEL - Personnalisation visuelle
    THEME: 'theme-green', // theme-green, theme-blue, theme-red
    LOGO_EMOJI: '🏢', // Emoji ou laisser vide pour logo custom
    
    // CONTACT
    CABINET_TELEPHONE: 'REMPLACER_PAR_TELEPHONE',
    CABINET_EMAIL: 'REMPLACER_PAR_EMAIL_CABINET',
    CABINET_ADRESSE: 'REMPLACER_PAR_ADRESSE_CABINET'
};
